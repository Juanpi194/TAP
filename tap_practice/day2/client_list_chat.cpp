...
void	handle_client(int client_fd, std::list<int>& client_list, std::mutex& mtx)
{
	...
	while (true)
	{
		...
		if (bytes == 0)
			break;
		broadcast(client_list, mtx, std::string(buffer, bytes));	
		// La diferencia es que std::string(buffer) lee hasta el primer \0,
		// mientras que std::string(buffer, bytes) usa exactamente los bytes que recv recibió. 
	}
    close(client_fd);
